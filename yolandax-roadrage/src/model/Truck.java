 ...
 * @version 25 Oct 2019
 */
public final class Truck extends AbstractVehicle implements Vehicle {
    
    /**
     * The number of updates between Truck's death and when it should revive.
     */
    private static final int MY_DEATH_ROUNDS = -1;  //Truck never dies
    
    /**
     * Constructor for a Truck.
     * @param theX X location on the map
     * @param theY Y location on the map
     * @param theDir direction the Truck is facing
     */
     ...
     * @return true if the truck can pass
     */
    @Override
    ...
    /**
     * Trucks randomly select to go straight, turn left, or turn right. Otherwise, reverse.
     * @return the direction
     */
    @Override
    public Direction chooseDirection(final Map<Direction, Terrain> theNeighbors) {
        Direction dir = getDirection();
        final boolean cantPass = !canPass(theNeighbors.get(dir), Light.GREEN) 
                        && !canPass(theNeighbors.get(dir.left()), Light.GREEN)
                        && !canPass(theNeighbors.get(dir.right()), Light.GREEN);
        if (cantPass) {
            dir = dir.reverse();
        } else {
            final Random rn = new Random();
